...
import core.planning.core.population.PlacesConnoisseur;
import core.planning.core.population.PlacesSharer;
import core.planning.core.population.agenda.Agenda;
...
public class SchedulingNetwork implements Network {
	...
	public class SchedulingNode extends NodeImpl implements Node {
		...
		public void updatePath(List<SchedulingLink> path, double utility) {
			if(this.utility<utility) {
				this.path = new ArrayList<SchedulingLink>(path);
	...
	private PassivePlannerManager.CurrentTime now;
	...
	public List<SchedulingLink> createNetwork(PassivePlannerManager.CurrentTime now, ActivityFacilities facilities, Id<ActivityFacility> originId, Id<ActivityFacility> destinationId,
											  ...
											  List<Tuple<String, Tuple<Double, Double>>> followingActivities, final PassivePlannerManager.MobsimStatus mobsimStatus) {
		...
		Map<Id<ActivityFacility>, PlacesConnoisseur.KnownPlace> knownPlaces = new HashMap<Id<ActivityFacility>, PlacesConnoisseur.KnownPlace>();
		placeSharer.setAreKnownPlacesUsed(true);
		for(PlacesConnoisseur.KnownPlace knownPlace:placeSharer.getKnownPlaces())
		...
		PlacesConnoisseur.KnownPlace place = knownPlaces.get(originId);
		...
		if(lastNode.path!=null) {
			...
			for(SchedulingLink link:lastNode.path)
				if(link instanceof ActivitySchedulingLink) {
					Agenda.AgendaElement agendaElement = agenda.getElements().get(((ActivitySchedulingLink)link).activityType);
	...
	private void addNodesAndLinks(SchedulingNode node, Id<ActivityFacility> originId, Set<String> modes, Id<ActivityFacility> carLocationId, Map<Id<ActivityFacility>, PlacesConnoisseur.KnownPlace> knownPlaces, Agenda agenda, List<SchedulingLink> path, boolean hadCar, final PassivePlannerManager.MobsimStatus mobsimStatus) {
			...
			if(pLink instanceof ActivitySchedulingLink || path.size()<maximumDepth) {
				...
				if(activityTypes!=null)
					for(String activityType:activityTypes) {
						Agenda.AgendaElement agendaElement = agenda.getElements().get(activityType);
				...
				if(pLink instanceof ActivitySchedulingLink) {
					...
					List<PlacesConnoisseur.KnownPlace> knownPlacesList = new ArrayList<>(knownPlaces.values());
					for(int i=0; i<knownPlacesList.size(); i++) {
						PlacesConnoisseur.KnownPlace knownPlace = knownPlacesList.get(i);
	...
	private boolean willingToPerform(Agenda.AgendaElement agendaElement, double duration, double numTimes) {
	...
	private double getUtility(List<SchedulingLink> path, Agenda agenda, boolean hadCar, double endTime) {
		...
		if(lastAct.equals(firstAct)) {
			Agenda.AgendaElement agendaElement = agenda.getElements().get(lastAct);
			...
			if(lastTimeF==null)
				lastTimeF = 0.0;
			Agenda.AgendaElement agendaElement = agenda.getElements().get(firstAct);
	...
	public static void main(String[] args) {
		...
		List<Tuple<String, Tuple<Double, Double>>> previousActivities = new ArrayList<>();
		...
		List<Tuple<String, Tuple<Double, Double>>> followingActivities = new ArrayList<>();
		...
		List<SchedulingLink> path = network.createNetwork(new PassivePlannerManager.CurrentTime(), scenario.getActivityFacilities(), originId, destinationId, Integer.parseInt(args[5]), new HashSet<String>(Arrays.asList("car", "pt", "walk")), placeSharer, agenda, previousActivities, followingActivities, new PassivePlannerManager.MobsimStatus());
