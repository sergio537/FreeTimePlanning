...
import core.planning.api.population.EmptyTime;
import core.planning.core.population.EmptyTimeImpl;
import core.planning.core.population.PlacesSharer;
import core.planning.core.population.agenda.Agenda;
import core.planning.core.population.decisionMakers.types.RouteDecisionMaker;
import core.planning.population.parallelPassivePlanning.PassivePlannerManager;
import core.scheduling.SchedulingNetwork;
...
public class AgendaDecisionMaker extends PlacesSharer implements RouteDecisionMaker {
	...
	private PassivePlannerManager.MobsimStatus mobsimStatus;
	...
	public void prepareScheduling(Activity activity, Plan plan, PassivePlannerManager.CurrentTime now) {
		this.now = now;
	...
	public List<? extends PlanElement> decideRoute(double time,	Id<ActivityFacility> startFacilityId, Id<ActivityFacility> endFacilityId, String mode,
		...
		List<SchedulingNetwork.SchedulingLink> path = new SchedulingNetwork().createNetwork(now, facilities, startFacilityId, endFacilityId,
		...
		if(path!=null) {
			for(SchedulingNetwork.SchedulingLink link:path)
				if(link instanceof SchedulingNetwork.ActivitySchedulingLink) {
					SchedulingNetwork.ActivitySchedulingLink activityLink = (SchedulingNetwork.ActivitySchedulingLink)link;
					ActivityImpl activity = new ActivityImpl(activityLink.getActivityType(), activityLink.getCoord());
					activity.setEndTime(((SchedulingNetwork.SchedulingNode)link.getToNode()).getTime());
					...
					SchedulingNetwork.JourneySchedulingLink journeyLink = (SchedulingNetwork.JourneySchedulingLink)link;
					...
					if(facilities.getFacilities().get(currentFacilityId).getLinkId().equals(facilities.getFacilities().
						...
						List<? extends PlanElement> routeElements = tripRouter.calcRoute(journeyLink.getMode(), facilities.getFacilities().
								...
								((SchedulingNetwork.SchedulingNode)link.getFromNode()).getTime(), null);
	...
	public void setMobsimEnds(final PassivePlannerManager.MobsimStatus mobsimStatus) {
