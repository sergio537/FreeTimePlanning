package core.planning.core.population.agenda;
...
public class Agenda {
	...
	public class AgendaElement {
		...
		public AgendaElement(String type, RealDistribution numTimesPerPeriod, RealDistribution duration) {
			...
			this.currentPerformedTime = numTimesPerPeriod.getNumericalMean()*duration.getNumericalMean();
		}
		...
		public void resetCurrentPerformedTime() {
			currentPerformedTime = numTimesPerPeriod.getNumericalMean()*duration.getNumericalMean();
		}
	...
	public void addElement(String type, double x) {
		AgendaElement element = new AgendaElement(type, new NormalDistribution(1, 0.5), new NormalDistribution(x, 3600));
		element.setNumObservations(1);
		elements.put(type, element);
	}
	public void addElement(String type, RealDistribution duration) {
		AgendaElement element = new AgendaElement(type, new NormalDistribution(1, 0.5), duration);
		element.setNumObservations(1);
		elements.put(type, element);
	}
	public void addElement(String type, double xA, double xB) {
		...
		AgendaElement element = new AgendaElement(type, new NormalDistribution(2, 1), new NormalDistribution(mean, sd));
		element.setNumObservations(2);
		elements.put(type, element);
	}
	public void addElement(String type, RealDistribution numTimesPeriod, RealDistribution duration) {
		AgendaElement element = new AgendaElement(type, numTimesPeriod, duration); 
		elements.put(element.getType(), element);
	}
	public void addElement(String type, double meanNumTimesPeriod, double sdNumTimesPeriod, double meanDuration, double sdDuration) {
		AgendaElement element = new AgendaElement(type, meanNumTimesPeriod, sdNumTimesPeriod, meanDuration, sdDuration); 
		elements.put(element.getType(), element);
	}
