...
public class PlacesConnoisseur {
	...
	public class KnownPlace {
		...
		private SortedMap<Time.Period, Set<String>> timeTypes = new TreeMap<>();
		/*{
			for(Period period:Time.Period.values())
				timeTypes.put(period, new HashSet<String>());
		}*/
		protected Map<String, SortedMap<Time.Period, Map<Id<ActivityFacility>, Double>>> travelTimes = new HashMap<>();
		...
		public Set<double[]> getTimes(String activityType) {
			...
			for(Map.Entry<Time.Period, Set<String>> timeType: timeTypes.entrySet())
	...
	public void addKnownPlace(KnownPlace knownPlace) {
		...
		if(knownPlace2==null)
			...
			for(Map.Entry<Time.Period, Set<String>> entry:knownPlace.timeTypes.entrySet()) {
